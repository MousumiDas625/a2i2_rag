#!/usr/bin/env python3
"""
exp9_random_persona_global_rag.py — Random policy + persona + global RAG
==========================================================================

PURPOSE:
    Each operator turn:
        1. Random training policy (for persona conditioning).
        2. Few-shot examples from the global successful-operator corpus
           (same retrieval as exp4 / rag_successful style), not tied to the
           random policy's local index.

USAGE:
    python experiments/exp9_random_persona_global_rag.py
    python experiments/exp9_random_persona_global_rag.py --residents ross,bob --runs 3
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Experiment 9: Random policy + persona + global successful RAG"
    )
    ...
    folder = f"exp9_random_persona_global_rag_{ts}" + (f"_{args.tag}" if args.tag else "")
    ...
    print("=" * 60)
    print("  EXPERIMENT 9 — RANDOM PERSONA + GLOBAL RAG")
    ...
    for resident in residents:
        for run_idx in range(1, args.runs + 1):
            rid = f"exp9_{resident}_run{run_idx}"
            print(f"\n--- {rid} ---")
            result = run_conversation(
                resident_name=resident,
                strategy="random_persona_global_rag",
                seed_text=args.seed,
            ...
            all_results.append({
                "experiment": "random_persona_global_rag",
                "resident": resident,
                "run": run_idx,
    ...
    summary = {
        "experiment": "exp9_random_persona_global_rag",
        "timestamp": ts,
        "results": all_results,
        "per_resident": {},
    }
    ...
    print(f"\n{'=' * 60}")
    print("  RANDOM PERSONA + GLOBAL RAG RESULTS")
    print(f"{'=' * 60}")
