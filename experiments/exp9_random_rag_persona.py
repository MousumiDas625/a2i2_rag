#!/usr/bin/env python3
"""
exp9_random_rag_persona.py — Experiment 9: Random Policy + Per-Policy RAG + Persona
=====================================================================================
...
PURPOSE:
    Baseline. At each operator turn:
        ...
        3. Per-policy FAISS RAG examples are retrieved for that policy.
        4. NO IQL.

    Comparing Exp9 vs Exp3 (IQL+RAG) isolates the value of IQL policy
    selection when both RAG and persona info are present.

USAGE:
    python experiments/exp9_random_rag_persona.py
    python experiments/exp9_random_rag_persona.py --residents ross,bob --runs 3
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Experiment 9: Random policy + per-policy RAG + persona"
    )
    ...
    folder = f"exp9_random_rag_persona_{ts}" + (f"_{args.tag}" if args.tag else "")
    ...
    print("=" * 60)
    print("  EXPERIMENT 9 — RANDOM POLICY + PER-POLICY RAG + PERSONA")
    ...
    summary = {
        "experiment": "exp9_random_rag_persona",
        "timestamp": ts,
        "results": all_results,
        "per_resident": {},
    }
    ...
    print(f"\n{'=' * 60}")
    print("  RANDOM POLICY + PER-POLICY RAG + PERSONA RESULTS")
    print(f"{'=' * 60}")
